...
pub trait AbsValue: Clone {
    fn abs_value(&self) -> Self;
}

macro_rules! impl_abs_value_for_signed {
    ($($t:ty),+ $(,)?) => {
        $(
            impl AbsValue for $t {
                fn abs_value(&self) -> Self {
...
impl_abs_value_for_signed!(f32, f64, i8, i16, i32, i64, i128, isize);

impl AbsValue for num_complex::Complex32 {
    fn abs_value(&self) -> Self {
        num_complex::Complex32::new(self.norm(), 0.0)
    }
}

impl AbsValue for num_complex::Complex64 {
    fn abs_value(&self) -> Self {
        num_complex::Complex64::new(self.norm(), 0.0)
    }
}

pub trait SignValue: Clone {
    fn sign_value(&self) -> Self;
}

macro_rules! impl_sign_value_for_signed {
    ($($t:ty),+ $(,)?) => {
        $(
            impl SignValue for $t {
                fn sign_value(&self) -> Self {
                    self.signum()
                }
            }
...
impl_sign_value_for_signed!(f32, f64, i8, i16, i32, i64, i128, isize);

impl SignValue for num_complex::Complex32 {
    fn sign_value(&self) -> Self {
...
impl SignValue for num_complex::Complex64 {
    fn sign_value(&self) -> Self {
        if self.re == 0.0 && self.im == 0.0 {
...
where
    T: Clone + Default + AbsValue + 'static,
{
    let mut output = Array::from_data(vec![T::default(); x.size()], x.shape().to_vec());
    for i in 0..x.size() {
        if let Some(val) = x.get(i) {
            output.set(i, val.abs_value())?;
        }
    }
    Ok(output)
}
...
where
    T: Clone + Default + SignValue + 'static,
{
    let mut output = Array::from_data(vec![T::default(); x.size()], x.shape().to_vec());
    for i in 0..x.size() {
        if let Some(val) = x.get(i) {
            output.set(i, val.sign_value())?;
        }
    }
    Ok(output)
}

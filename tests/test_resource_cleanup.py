...
class TestResourceCleanup:
    ...
    def test_memory_usage_monitoring():
        ...
        model.generate_audio(
            model_state=state,
            text_to_generate="Test memory monitoring",
        )
    ...
    def test_prompt_cache_cleanup():
        """Test that prompt cache can be cleared."""
        model = TTSModel.load_model()
        
        # Add some entries to cache
        state1 = model.init_states()
        state2 = model.init_states()
    ...
    def test_consecutive_generations_no_leak():
        ...
        for i in range(3):
            ...
            audio = model.generate_audio(
                model_state=state,
                text_to_generate=f"Generation {i}",
            )
            assert audio is not None
    ...
    def test_cuda_cache_cleanup():
        ...
        model.generate_audio(
            model_state=state,
            text_to_generate="Test CUDA cleanup",
        )
    ...
    def test_memory_usage_logging(caplog):

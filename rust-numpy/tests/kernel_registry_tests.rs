...
mod kernel_registry_tests {
    ...
    fn test_multiple_kernel_registration() {
        ...
        registry.register(UfuncType::Add, F64AddKernel).unwrap();
        registry.register(UfuncType::Multiply, F64MulKernel).unwrap();
...
mod kernel_performance_tests {
    ...
    fn test_kernel_execution_performance() {
        ...
        println!("Kernel execution time for {} elements: {:?}", size, duration);
    }
    ...
    fn test_multiple_kernel_types_performance() {
        ...
        f64_kernel.execute(&[&a_f64, &b_f64], &mut output_f64).unwrap();
        ...
        f32_kernel.execute(&[&a_f32, &b_f32], &mut output_f32).unwrap();
        ...
        i32_kernel.execute(&[&a_i32, &b_i32], &mut output_i32).unwrap();

...
## Issue #141: Implement comprehensive unit test coverage for TTSModel

Create unit tests for all core methods in `TTSModel` class in `pocket_tts/models/tts_model.py`.

Acceptance criteria:
- Test prompt caching mechanism
- Test state management for streaming generation
- Test EOS detection and post-EOS frame handling
- Test voice state encoding/decoding
- Test streaming buffer management
- All tests should pass with pytest -n 3 -v

Implementation notes:
- Use pytest fixtures for model initialization
- Mock MimiModel and FlowLMModel for isolated testing
- Test concurrent streaming behavior
- Verify state preservation across multiple generations

---

## Issue #142: Add integration tests for CLI audio generation workflow

Create integration tests that verify the complete CLI audio generation pipeline.

Acceptance criteria:
- Test `pocket-tts generate` command end-to-end
- Test with multiple voices (alba, hifi, etc.)
- Test output file verification (wav format, audio quality)
- Test error handling for invalid inputs
- Verify CLI arguments parsing and validation

Implementation details:
- Use tempfile for output file testing
- Verify generated audio using librosa or similar
- Test with various input strings (short, long, special chars)
- Validate help message and error messages

---

## Issue #143: Improve error handling in FlowLM model generation

Enhance error handling and validation in `FlowLMModel.generate()` method.

Acceptance criteria:
- Add input validation for text length, voice selection
- Implement proper exception handling with informative error messages
- Add validation for model state (pre-check before generation)
- Ensure safe fallback behavior on generation failures
- Log detailed error context for debugging

Implementation details:
- Use existing exception patterns from MimiModel
- Create custom exceptions for generation failures
- Add logging for all error paths
- Verify error messages are user-friendly

---

## Issue #147: Add performance profiling for streaming generation

Implement performance profiling to identify bottlenecks in streaming generation.

Acceptance criteria:
- Profile streaming generation for various text lengths
- Identify CPU hotspots in FlowLM and Mimi processing
- Compare streaming vs batch processing performance
- Document optimization opportunities
- Measure frame generation latency

Implementation details:
- Use Python's cProfile or similar tools
- Profile both model generation and audio processing
- Generate performance reports
- Identify critical paths for optimization

---

## Issue #148: Add audio encoding/decoding validation tests

Create tests to verify audio encoding and decoding integrity in Mimi codec.

Acceptance criteria:
- Test round-trip encoding/decoding (lossless)
- Verify audio quality after encode/decode cycles
- Test various audio formats and bitrates
- Check for sample rate conversion accuracy
- Validate audio metadata preservation

Implementation details:
- Generate test audio signals
- Encode then decode and compare
- Use objective audio quality metrics
- Test edge cases (silence, peaks, noise)

---

## Issue #149: Enhance prompt state caching efficiency

Optimize the prompt state caching mechanism for better performance.

Acceptance criteria:
- Benchmark cache hit/miss rates
- Optimize cache storage format
- Verify cache doesn't grow unbounded
- Test concurrent prompt caching
- Ensure thread-safe access to cached states

Implementation details:
- Analyze current caching performance
- Consider cache eviction policies
- Implement any necessary optimizations
- Test with high volume of prompts

---

## Issue #150: Improve error messages for user-facing APIs

Enhance error messages in the main.py FastAPI server and CLI interface.

Acceptance criteria:
- Provide clear, actionable error messages
- Include helpful troubleshooting steps
- Format errors consistently
- Translate technical jargon for end users
- Add example usage in error messages when applicable

Implementation details:
- Review all API endpoints for error handling
- Update CLI error messages
- Add user-friendly error templates
- Test with expected and unexpected errors

---

## Issue #151: Add WebSocket support to web API

Implement WebSocket streaming support for real-time audio generation in the FastAPI server.

Acceptance criteria:
- Create WebSocket endpoint for audio streaming
- Support real-time generation for text input
- Handle connection management and cleanup
- Implement heartbeat/ping for connection monitoring
- Test with browser clients

Implementation details:
- Use FastAPI WebSocket handlers
- Stream audio frames as they are generated
- Handle disconnections gracefully
- Document WebSocket API usage

---

## Issue #152: Add model versioning and backward compatibility

Implement model versioning to support multiple model versions.

Acceptance criteria:
- Add version metadata to model files
- Implement version detection in model loader
- Add backward compatibility layer for different versions
- Support loading multiple model versions simultaneously
- Document version compatibility matrix

Implementation details:
- Update model file format with version info
- Modify model loader to detect and handle versions
- Add version-specific logic where needed
